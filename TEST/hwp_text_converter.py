...
def find_hwp_documents(directory: str) -> List[str]:
    return [os.path.join(directory, fname) for fname in os.listdir(directory)
            if fname.lower().endswith('.hwp')]


def save_markdown_to_file(md_text: str, output_path: str) -> None:
    with open(output_path, 'w', encoding='utf-8') as fw:
        fw.write(md_text)


def process_hwp_documents(directory: str = '.') -> None:
    hwps = find_hwp_documents(directory)
    if not hwps:
        print(f"{directory}: HWP 파일 없음.")
        return
    for hwp in hwps:
        base = os.path.splitext(os.path.basename(hwp))[0]
        md_path = os.path.join(directory, base + '.md')
        print(f"처리 중: {os.path.basename(hwp)} -> {base}.md")
        try:
            md = get_hwp_text(hwp)
            save_markdown_to_file(md, md_path)
            print(f"완료: {md_path}")
...
def main():
    parser = argparse.ArgumentParser(
        description='HWP 문서를 Markdown(.md)으로 변환하는 도구 (표 유지 강화)')
    parser.add_argument('-d', '--directory', default='.',
                        help='처리할 디렉토리 (기본: 현재)')
    args = parser.parse_args()
    process_hwp_documents(args.directory)
